...
public class LineChartM3 extends Application {

	private static ObservableList<XYChart.Data<String, Number>> createXYDataSet(String[] label, Number[] data, int year) {
		ObservableList<XYChart.Data<String, Number>> list = FXCollections.observableArrayList();

		for (int i = 0; i < data.length; i++) {
			list.add(new XYChart.Data<String, Number>(label[i] + ", " + year, data[i].doubleValue()));
	...
	public void start(Stage stage) throws Exception {
		...
		lineChartData = createXYDataSet(months, y2012, 2012);
		ObservableList<XYChart.Data<String, Number>> lineChartData2;
		lineChartData2 = createXYDataSet(months, y2013, 2013);
		ObservableList<XYChart.Data<String, Number>> lineChartData3;
		lineChartData3 = createXYDataSet(months, y2014, 2014);
		
		ObservableList<XYChart.Data<String, Number>> completeData = FXCollections.observableArrayList();
		completeData.addAll(lineChartData);
		completeData.addAll(lineChartData2);
		completeData.addAll(lineChartData3);


		XYChart.Series<String, Number> series1 = new XYChart.Series<>(completeData);
		series1.setName("2012-2014");
